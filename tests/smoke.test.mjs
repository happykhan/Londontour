...
test('app uses a real online basemap, local offline fallback, layer registry hooks, and both routes', () => {
  ...
  assert.match(js, /const lineWeight = isSelected && selectedLineIds\.size > 1 \? 5\.2 : isSelected \? 6\.4 : 4\.4/);
  assert.match(js, /const lineOpacity = isDimmed \? 0\.48 : isSelected \? 1 : 0\.95/);
  assert.match(js, /const casing = \{/);
  assert.match(js, /overlayOrder: 28/);
  assert.match(js, /overlayOrder: 29/);
  assert.match(js, /weight: lineWeight \+ 3\.6/);
  assert.match(js, /tubeLineLayers\.push\(casingPolyline, polyline\)/);
  assert.match(js, /opacity: selectedLineIds\.size \? 0\.46 : 0\.76/);
  assert.match(js, /overlayOrder: 27/);
...
test('selected multi-line tube stations render offset line bands', () => {
  ...
  assert.match(js, /selectedLineIds\.size > 1 \? 5\.2/);
  assert.match(js, /const casingPolyline = L\.polyline\(segments, casing\)/);
});

...
test('index renders the route picker and offline controls', () => {
  ...
  assert.match(html, /id="radius-lock-button"/);
  ...
  assert.match(html, />Offline download<\/button>/);
  assert.match(html, /id="menu-about-button"/);
  assert.match(html, />About &amp; licences<\/button>/);
  assert.doesNotMatch(html, /id="basemap-proof-link"/);
  assert.doesNotMatch(html, /id="print-button"/);
  assert.match(html, /<section id="about-panel"/);
  assert.match(html, /id="about-back-button"/);
  assert.match(html, /id="about-close-button"/);
  ...
  assert.match(html, /id="offline-back-button"/);
  assert.match(html, /id="offline-close-button"/);
  ...
  assert.match(html, /id="start-guide-button" type="button" aria-pressed="false"/);
  ...
  assert.match(html, /serviceWorker\.register\('\/sw\.js\?v=20260701-guide'\)/);
  assert.match(html, /assets\/vendor\/maplibre\/maplibre-gl\.js\?v=20260701-guide/);
  assert.match(html, /assets\/vendor\/maplibre\/maplibre-gl\.css\?v=20260701-guide/);
  assert.match(html, /assets\/vendor\/pmtiles\/pmtiles\.js\?v=20260701-guide/);
  assert.match(html, /assets\/maplibre-leaflet-adapter\.js\?v=20260701-guide/);
  assert.match(html, /data-editor-mode="draw"/);
  assert.match(html, /data-editor-mode="edit"/);
  assert.match(html, /data-editor-mode="insert"/);
  assert.match(html, /id="editor-inspector"/);
  assert.match(html, /id="editor-redo-button"/);
  assert.match(html, /id="editor-download-button"/);
...
test('app uses a real online basemap, local offline fallback, layer registry hooks, and both routes', () => {
  ...
  assert.match(js, /const menuCloseButton = document\.querySelector\('#menu-close-button'\)/);
  ...
  assert.match(js, /const menuAboutButton = document\.querySelector\('#menu-about-button'\)/);
  assert.match(js, /const aboutBackButton = document\.querySelector\('#about-back-button'\)/);
  assert.match(js, /const aboutCloseButton = document\.querySelector\('#about-close-button'\)/);
  ...
  assert.match(js, /const radiusLockButton = document\.querySelector\('#radius-lock-button'\)/);
  assert.match(js, /let searchResults = \[\]/);
  assert.match(js, /let radiusState = \{/);
  assert.match(js, /locked: false/);
  assert.match(js, /selectedResultId: ''/);
  ...
  assert.match(js, /function keepRadiusCenterVisible/);
  assert.match(js, /function radiusSelectionPlaceholder/);
  ...
  assert.match(js, /const assetVersion = '20260701-guide'/);
  assert.match(js, /const cacheName = 'londontour-offline-v106'/);
  ...
  assert.match(js, /function parseSharedPoiParam/);
  assert.match(js, /const initialSharedPoi = parseSharedPoiParam\(initialSearchParams\.get\('poi'\)\)/);
  assert.match(js, /function findLayerSearchItem/);
  ...
  assert.match(js, /let guideState = \{/);
  assert.match(js, /function guideEvents/);
  assert.match(js, /async function buildGuideRoutePoints/);
  assert.match(js, /function projectToGuideSegment/);
  assert.match(js, /function snapToGuideRoute/);
  assert.match(js, /function isGuideEventDue/);
  assert.match(js, /function guideNextEvent/);
  assert.match(js, /async function requestGuideWakeLock/);
  assert.match(js, /async function releaseGuideWakeLock/);
  assert.match(js, /function speakGuideText/);
  assert.match(js, /function triggerGuideEvent/);
  assert.match(js, /function guideOffRouteThreshold/);
  assert.match(js, /function warnIfGuideOffRoute/);
  assert.match(js, /function handleGuidePosition/);
  assert.match(js, /function stopGuideMode/);
  assert.match(js, /async function startGuideMode/);
  assert.match(js, /navigator\.geolocation\.watchPosition\(handleGuidePosition, handleGuideError/);
  assert.match(js, /navigator\.geolocation\.clearWatch\(guideState\.watchId\)/);
  assert.match(js, /new SpeechSynthesisUtterance/);
  assert.match(js, /wakeLock\.request\('screen'\)/);
  assert.match(js, /distanceFromRouteM/);
  assert.match(js, /offRouteSince/);
  assert.match(js, /guide-mute-button/);
  assert.match(js, /guide-map-button/);
  ...
  assert.match(js, /function validateRouteDraft/);
  assert.match(js, /function pushEditorHistory/);
  assert.match(js, /function undoEditorAction/);
  assert.match(js, /function redoEditorAction/);
  assert.match(js, /function setEditorUiMode/);
  assert.match(js, /function selectEditorPoint/);
  assert.match(js, /function deleteEditorPoint/);
  assert.match(js, /function insertEditorPointAfter/);
  assert.match(js, /function attachEditorHandleEvents/);
  ...
  assert.match(js, /uses unknown segment type/);
  assert.match(js, /Guide event/);
  assert.match(js, /function focusValidationIssue/);
  assert.match(js, /data-validation-type/);
  assert.match(js, /validation error/);
  ...
  assert.match(js, /clearTubeSelectionBeforePopup\(\);\n\s+document\.body\.classList\.remove\('route-menu-open', 'offline-menu-open', 'menu-open', 'about-menu-open'\)/);
  ...
  assert.match(adapter, /setLatLng\(point\) \{/);
  assert.match(adapter, /this\.marker\.setLngLat\(toLngLat\(point\)\)/);
  ...
  assert.match(adapter, /'line-opacity': 0,/);
...
test('nearby radius drag and marker selection keep their interaction state', () => {
  ...
  const renderLayerMarkers = js.slice(js.indexOf('function renderLayerMarkers'), js.indexOf('function elevateMarker'));
  ...
  assert.match(elevateMarkerHelper, /marker\?\.getElement\?\.\(\) \|\| marker\?\.element/);

  const adapter = read('assets/maplibre-leaflet-adapter.js');
  assert.match(adapter, /getElement\(\) \{ return this\.element; \}/);
  assert.match(adapter, /setZIndexOffset\(offset = 0\)/);
});
...
test('dark mode has explicit mobile surfaces and controls', () => {
  ...
  assert.match(css, /body\.route-view\.offline-menu-open \.offline-panel/);
  assert.match(css, /body\.route-view\.about-menu-open \.tour-panel/);
  assert.match(css, /body\.route-view\.about-menu-open \.about-panel/);
  assert.match(css, /body\.route-view\.menu-open \.menu-panel/);
  ...
  assert.match(html, /class="lock-shackle lock-shackle-open"/);
  assert.match(html, /class="lock-shackle lock-shackle-closed"/);
  assert.match(css, /\.radius-lock-button\[aria-pressed="true"\] \.lock-shackle-open/);
  assert.match(css, /\.radius-lock-button\[aria-pressed="true"\] \.lock-shackle-closed/);
  ...
  assert.match(css, /\.guide-warning/);
  assert.match(css, /\.validation-summary button/);
  assert.match(css, /\.editor-mode-tabs/);
  assert.match(css, /\.editor-inspector/);
  ...
  assert.match(css, /\.map-popup-card/);
  assert.match(css, /\.poi-popup/);
  assert.match(css, /\.poi-popup-copy/);
  ...
  assert.match(css, /max-inline-size: min\(84vw, 22rem\)/);
  ...
  assert.match(css, /\.leaflet-popup-content-wrapper/);
  assert.doesNotMatch(css, /max-height: 9\.5rem/);
...
test('service worker precaches the local tile pack', () => {
  const sw = read('sw.js');
  assert.match(sw, /londontour-offline-v106/);
...
test('offline basemap manifest can drive the download button', () => {
  ...
  assert.equal(manifest.version, '20260623-1220');
...
test('MapLibre PMTiles proof page is wired to self-hosted London archive', () => {
  ...
  assert.match(html, /assets\/maplibre-poc\.js\?v=20260623-1220/);
